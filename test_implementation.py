...
def test_mock_data_generation():
    ...
    if mock_items:
        ...
        console.print(f"✓ Sample item has required fields: id={sample_item['id']}, title={sample_item['title'][:30]}...")
        console.print(f"✓ Item has {len(sample_item.get('transitions', []))} state transitions")
...
def test_metrics_calculation(work_items):
    ...
    if metrics.get('lead_time'):
        table.add_row("Average Lead Time", f"{metrics['lead_time']['average']:.1f} days")
        table.add_row("Median Lead Time", f"{metrics['lead_time']['median']:.1f} days")
    
    if metrics.get('cycle_time'):
        table.add_row("Average Cycle Time", f"{metrics['cycle_time']['average']:.1f} days")
        table.add_row("Median Cycle Time", f"{metrics['cycle_time']['median']:.1f} days")
    
    if metrics.get('throughput'):
        table.add_row("Throughput (30 days)", f"{metrics['throughput']['per_30_days']:.1f} items")
    
    if metrics.get('flow_efficiency'):
        table.add_row("Flow Efficiency", f"{metrics['flow_efficiency']:.1f}%")
    
    if metrics.get('littles_law'):
        table.add_row("Little's Law Variance", f"{metrics['littles_law']['variance']:.1f}%")
    ...
    if report.get('team_metrics'):
        ...
        for member in report['team_metrics']:
            team_table.add_row(
                ...
                f"{member.get('average_lead_time', 0):.1f} days" if member.get('average_lead_time') else "N/A"
            )
...
def test_configuration():
    ...
    try:
        ...
        if not config_path.exists():
            console.print("[yellow]⚠ config.json not found, using sample configuration[/yellow]")
            ...
            test_config = {
                ...
                "stage_metadata": [
                    {"stage_name": "New", "stage_group": "Backlog", "is_active": False, "is_done": False},
                    {"stage_name": "In Progress", "stage_group": "Development", "is_active": True, "is_done": False},
                    {"stage_name": "Done", "stage_group": "Completed", "is_active": False, "is_done": True}
                ]
            }
        ...
        console.print(f"✓ Found {len(active_stages)} active stages: {', '.join(active_stages[:3])}...")
        console.print(f"✓ Found {len(done_stages)} done stages: {', '.join(done_stages[:3])}...")
