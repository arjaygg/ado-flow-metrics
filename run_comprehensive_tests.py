...
class TestPyramidRunner:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.quality_gates = {
            ...
            "min_integration_test_coverage": 75.0,
            "min_overall_coverage": 80.0,
            "max_unit_test_time": 60.0,  # seconds
            "max_integration_test_time": 300.0,  # seconds
    ...
    def run_all_tests(self, 
                     ...
                     generate_coverage: bool = True) -> bool:
    ...
    def _execute_test_command(self, cmd: List[str], test_type: str) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                failed=1,
                skipped=0,
                execution_time=self.quality_gates.get(f"max_{test_type}_test_time", 600),
                exit_code=1
            )
        except Exception as e:
    ...
    def _check_quality_gate(self, result: TestResult, test_type: str) -> bool:
        ...
        if result.failed > 0:
            print(f"❌ Quality gate failed: {test_type} tests have {result.failed} failures")
            return False
        ...
        if max_time_key in self.quality_gates:
            if result.execution_time > self.quality_gates[max_time_key]:
                print(f"⚠️ Quality gate warning: {test_type} tests took {result.execution_time:.2f}s "
        ...
        if min_coverage_key in self.quality_gates and result.coverage_percentage > 0:
            if result.coverage_percentage < self.quality_gates[min_coverage_key]:
                print(f"❌ Quality gate failed: {test_type} coverage {result.coverage_percentage:.1f}% "
    ...
    def _assess_overall_quality(self) -> bool:
        ...
        if total_failed > 0:
            print(f"❌ Overall quality assessment failed: {total_failed} total test failures")
            return False
        ...
        if unit_percentage < 50:  # At least 50% should be unit tests
            print(f"⚠️ Test pyramid shape warning: Only {unit_percentage:.1f}% unit tests")
        
        # Check overall coverage
        overall_coverage = self._calculate_overall_coverage()
        if overall_coverage < self.quality_gates["min_overall_coverage"]:
            print(f"❌ Overall coverage {overall_coverage:.1f}% below threshold "
    ...
    def _calculate_overall_coverage(self) -> float:
        ...
        coverages = [r.coverage_percentage for r in self.results if r.coverage_percentage > 0]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run ADO Flow Hive comprehensive test pyramid")
