...
class CLIParser:
    ...
    def validate_fetch_options(self, **kwargs) -> Dict[str, Any]:
        ...
        if output:
            ...
            if not output_path.parent.exists():
                raise ConfigurationError(f"Output directory does not exist: {output_path.parent}")
    ...
    def validate_calculate_options(self, **kwargs) -> Dict[str, Any]:
        ...
        if output_format not in valid_formats:
            raise ConfigurationError(f"Invalid output format: {output_format}. Must be one of: {', '.join(valid_formats)}")
