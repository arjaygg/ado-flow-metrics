...
class FlowMetricsErrorHandler:
    """Centralized error handler for Flow Metrics application with enhanced features."""
    ...
    def handle_error(
        ...
        response = self._generate_error_response(
            error_id, error_type, error_details, user_message
        )
    ...
    def handle_data_source_error(
        ...
        return self.handle_error(
            error, ErrorType.DATA_SOURCE_ERROR, context, user_message
        )
    ...
    def handle_cache_error(
        ...
        return self.handle_error(error, ErrorType.CACHE_ERROR, context, user_message)

    def handle_api_error(
        self, error: Exception, api_endpoint: str
    ) -> Tuple[Dict[str, Any], int]:
        """Handle external API errors."""
        context = {"api_endpoint": api_endpoint}
        user_message = (
            f"External API call to {api_endpoint} failed. Please try again later."
        )

        return self.handle_error(
            error, ErrorType.EXTERNAL_API_ERROR, context, user_message
        )
    ...
    def handle_file_io_error(
        ...
        return self.handle_error(error, ErrorType.FILE_IO_ERROR, context, user_message)

    def handle_validation_error(
        self, error: Exception, field: str = None
    ) -> Tuple[Dict[str, Any], int]:
        ...
        return self.handle_error(
            error, ErrorType.VALIDATION_ERROR, context, user_message
        )

    def handle_configuration_error(
        self, error: Exception, config_key: str = None
    ) -> Tuple[Dict[str, Any], int]:
        ...
        return self.handle_error(
            error, ErrorType.CONFIGURATION_ERROR, context, user_message
        )
    ...
    def _extract_error_details(
        self, error: Exception, error_type: str, context: Optional[Dict]
    ) -> Dict[str, Any]:
    ...
    def _log_error(
        self, error_id: str, error_details: Dict[str, Any], error: Exception
    ):
        ...
        if error_details["context"]:
            logger.log(
                log_level, f"Error {error_id} context: {error_details['context']}"
            )
...
def handle_error_with_response(
    error: Exception, error_type: str = ErrorType.INTERNAL_ERROR, **kwargs
):
    """
