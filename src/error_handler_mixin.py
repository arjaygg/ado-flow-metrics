...
class ErrorHandlerMixin:
    ...
    def handle_api_error(self, error: Exception, context: str = "API") -> None:
        ...
        if isinstance(error, AuthenticationError):
            ...
            self._print_error_if_available("Authentication failed - check your PAT token")
            self._suggest_mock_data()
            
        elif isinstance(error, AuthorizationError):
            logger.error(f"{context} authorization failed: {error_msg}")
            self._print_error_if_available("Access forbidden - check project permissions or conditional access policies")
            self._suggest_mock_data()
            ...
            if "405" in error_msg or "Method Not Allowed" in error_msg:
                self._print_warning_if_available("This usually indicates an API endpoint issue or authentication problem")
            self._suggest_mock_data()
    ...
    def handle_validation_error(self, error: Exception, context: str = "Validation") -> None:
        """Handle validation errors."""
        error_msg = str(error)
        logger.error(f"{context} validation failed: {error_msg}")
        self._print_error_if_available(f"Validation Error: {error_msg}")
    
    def handle_file_error(self, error: Exception, file_path: str, operation: str = "file operation") -> None:
        ...
        if "Permission denied" in error_msg:
            ...
            self._print_info_if_available("Check file permissions and try running as administrator if needed")
        elif "No such file or directory" in error_msg:
            self._print_error_if_available(f"File not found: {file_path}")
        elif "Is a directory" in error_msg:
            self._print_error_if_available(f"Expected file but found directory: {file_path}")
        else:
    ...
    def handle_network_error(self, error: Exception, context: str = "Network") -> None:
        ...
        if "timeout" in error_msg.lower():
            self._print_error_if_available("Network timeout - check your internet connection")
            self._print_info_if_available("Try increasing timeout or checking network connectivity")
        elif "connection" in error_msg.lower():
            self._print_error_if_available("Connection error - unable to reach Azure DevOps")
            self._print_info_if_available("Check your network connection and firewall settings")
        else:
    ...
    def safe_execute(self, func, *args, error_context: str = "Operation", **kwargs):
        ...
        try:
            ...
            self._print_error_if_available(f"Unexpected error during {error_context.lower()}: {e}")
            return None
    ...
    def _suggest_mock_data(self) -> None:
        ...
        self._print_info_if_available("For immediate testing, use mock data: python -m src.cli data fresh --use-mock")
