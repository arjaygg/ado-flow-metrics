...
from .error_handler import ErrorType, error_handler
...
class FlowMetricsWebServer:
    ...
    def _setup_routes(self):
        ...
        @self.app.route("/api/work-items")
        def get_work_items():
            """API endpoint to get work items data."""
            try:
                raw_work_items = self._load_work_items()
                ...
                for item in raw_work_items:
                    transformed_item = {
                        ...
                        "workItemType": item.get(
                            "type", item.get("work_item_type", "")
                        ),
                        "priority": item.get("priority", 0),
                        "assignedTo": item.get("assigned_to", ""),
                        "state": item.get("current_state", item.get("state", "")),
                        "createdDate": item.get("created_date", ""),
                        "tags": item.get("tags", []),
                        "storyPoints": item.get("story_points"),
                        "effortHours": item.get("effort_hours"),
                        "stateTransitions": item.get("state_transitions", []),
                    }
                    transformed_items.append(transformed_item)
                return jsonify(transformed_items)
            except Exception as e:
                return error_handler.create_flask_response(
                    *error_handler.handle_data_source_error(e, "work-items")
                )
        ...
        def serve_config(filename):
            ...
            try:
                ...
                if not str(requested_path.resolve()).startswith(
    ...
    def _load_work_items(self) -> list:
        """Load work items from the specified data source."""
        if self.data_source == "mock":
            # Generate mock data and return raw work items
            return generate_mock_azure_devops_data()
            ...
            try:
                ...
                error_handler.handle_api_error(e, "Azure DevOps API")
                # Fallback to cached data or mock data
                cached_data = self._load_work_items_from_cache()
                if cached_data:
                    return cached_data
                else:
                    # Fallback to mock data
        ...
        elif self.data_source.endswith(".json") or self.data_source == "file":
            # Load from JSON file
            data_paths = [
                Path("data/work_items.json"),
                Path("data/work_items_ado.json"),
                Path(self.data_source) if self.data_source.endswith(".json") else None,
            ]
            ...
            # If no file found, fallback to mock data
            return generate_mock_azure_devops_data()
    ...
    def _load_work_items_from_cache(self) -> Optional[list]:
        """Load work items from cache file."""
        try:
            cache_file = Path("data/cached_work_items.json")
            if cache_file.exists():
                with open(cache_file, "r") as f:
                    cache_data = json.load(f)
                    return cache_data.get("work_items", [])
        except Exception as e:
            # Use centralized error handling for cache errors
            error_handler.handle_cache_error(e, "load work items")
