...
class AzureDevOpsClient:
    ...
    def _get_state_history(self, work_item_id: int) -> List[Dict]:
        ...
        try:
                ...
                if 'System.State' in fields:
                    ...
                    if 'newValue' in state_info:
                        transition = {
                            ...
                            'date': update.get('fields', {}).get('System.ChangedDate', {}).get('newValue', ''),
                            'assigned_to': update.get('fields', {}).get('System.AssignedTo', {}).get('newValue', {}).get('displayName', '')
                        }
            ...
            print(f"Error fetching state history for work item {work_item_id}: {str(e)}")
            return []
    
    def get_team_members(self) -> List[str]:
        """Get team members from Azure DevOps"""
        try:
            teams_url = f"{self.org_url}/_apis/projects/{self.project}/teams?api-version=6.0"
            ...
            return [member.get('identity', {}).get('displayName', '') for member in members]
...
def load_azure_devops_data():
    ...
    try:
        ...
        if not pat_token or pat_token == 'YOUR_PAT_TOKEN_HERE':
            ...
            print("Option 1: Set environment variable: export AZURE_DEVOPS_PAT=your_token_here")
...
if __name__ == "__main__":
    ...
    if work_items:
        ...
        print("Failed to load Azure DevOps data. Check your configuration and PAT token.")