...
class FlowMetricsDatabase:
    ...
    def _initialize_database(self):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_transitions_date ON state_transitions(transition_date)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_flow_metrics_period ON flow_metrics(period_start, period_end)"
            )
    ...
    def cleanup_old_data(self, days_to_keep: int = 365):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                "SELECT id FROM executions WHERE timestamp < ?", (cutoff_date,)
            )
