...
class WorkItem(BaseModel):
    ...
    activated_date: Optional[datetime] = Field(
        None, description="First activation date"
    )
    ...
    def get_time_in_state(self, state_name: str) -> Optional[float]:
        ...
        for i, transition in enumerate(self.transitions):
            if transition.to_state == state_name:
                ...
                time_in_state += (
                    transition.transition_date - entered_state
                ).total_seconds() / 3600
                entered_state = None
...
class FlowMetrics(BaseModel):
    ...
    # Lead time metrics
    average_lead_time: Optional[float] = Field(
        None, description="Average lead time in days"
    )
    median_lead_time: Optional[float] = Field(
        None, description="Median lead time in days"
    )
    lead_time_percentiles: Dict[str, float] = Field(
        default_factory=dict, description="Lead time percentiles"
    )

    # Cycle time metrics
    average_cycle_time: Optional[float] = Field(
        None, description="Average cycle time in days"
    )
    median_cycle_time: Optional[float] = Field(
        None, description="Median cycle time in days"
    )
    cycle_time_percentiles: Dict[str, float] = Field(
        default_factory=dict, description="Cycle time percentiles"
    )
    ...
    wip_by_state: Dict[str, int] = Field(
        default_factory=dict, description="WIP breakdown by state"
    )
    wip_by_assignee: Dict[str, int] = Field(
        default_factory=dict, description="WIP breakdown by assignee"
    )

    # Flow efficiency
    flow_efficiency: Optional[float] = Field(
        None, ge=0, le=100, description="Flow efficiency percentage"
    )

    # Little's Law
    littles_law_cycle_time: Optional[float] = Field(
        None, description="Theoretical cycle time from Little's Law"
    )
    littles_law_variance: Optional[float] = Field(
        None, description="Variance from Little's Law prediction"
    )

    # Additional metrics
    blocked_items: int = Field(0, description="Number of blocked items")
    items_by_type: Dict[str, int] = Field(
        default_factory=dict, description="Items breakdown by type"
    )
...
class FlowMetricsReport(BaseModel):
    ...
    team_metrics: List[TeamMemberMetrics] = Field(
        default_factory=list, description="Per-member metrics"
    )
    ...
    bottlenecks: Optional[List[Dict[str, Any]]] = Field(
