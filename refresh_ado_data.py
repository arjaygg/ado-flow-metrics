...
def refresh_ado_data(days_back=30, output_path=None, verbose=False):
    ...
    try:
        logger.info(
            f"Starting Azure DevOps data refresh (looking back {days_back} days)"
        )
        ...
        # Fetch work items
        logger.info(
            f"Fetching work items from {ado_config.org_url}/{ado_config.default_project}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Refresh Azure DevOps Integration Data"
    )
    parser.add_argument(
    ...
    success = refresh_ado_data(
        days_back=args.days, output_path=args.output, verbose=args.verbose
    )
