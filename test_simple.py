...
def test_mock_data():
    ...
    if mock_items:
        ...
        print(f"✓ Sample has {len(sample.get('state_transitions', []))} state transitions")
...
def test_calculator():
    ...
    try:
        ...
        if throughput:
            print(f"✓ Throughput: {throughput.get('items_per_period', 0):.1f} items/{throughput.get('period_days', 30)} days")
        
        if report.get('flow_efficiency'):
            print(f"✓ Flow efficiency: {report['flow_efficiency'].get('efficiency_percentage', 0):.1f}%")
