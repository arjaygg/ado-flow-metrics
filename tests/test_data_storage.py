...
class TestFlowMetricsDatabase:
    ...
    def test_database_initialization(self, database):
        ...
        with database._get_connection() as conn:
            ...
            expected_tables = ['executions', 'work_items', 'state_transitions', 'flow_metrics']
    ...
    def test_store_flow_metrics(self, database, sample_flow_metrics):
        ...
        with database._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM flow_metrics WHERE execution_id = ?", (execution_id,))
            row = cursor.fetchone()
    ...
    def test_cleanup_old_data(self, database):
        ...
        with database._get_connection() as conn:
            ...
            cursor.execute("UPDATE executions SET timestamp = ? WHERE id = ?", (old_date, old_execution_id))
            conn.commit()
    ...
    def test_json_field_serialization(self, database):
        ...
        assert stored_item['custom_fields'] == {"priority_score": 85, "customer_id": "CUST-123"}