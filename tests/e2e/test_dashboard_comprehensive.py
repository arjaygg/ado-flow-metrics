...
class TestDashboardE2E:
    ...
    async def test_metrics_cards_display(self, page: Page, base_url: str):
        ...
        for card_id in metrics_cards:
            card = await page.wait_for_selector(f"[data-testid='{card_id}']", timeout=5000)
            ...
            assert text_content and text_content.strip(), f"Card {card_id} has no content"
    ...
    async def test_charts_render(self, page: Page, base_url: str):
        ...
        for selector in chart_selectors:
            try:
                ...
                assert svg_content or canvas_content, f"Chart {selector} has no visual content"
    ...
    async def test_filters_functionality(self, page: Page, base_url: str):
        ...
        date_filter = await page.wait_for_selector("[data-testid='date-range-filter']", timeout=5000)
        if date_filter:
            ...
            url = page.url
            assert "days=30" in url or await page.get_attribute("[data-testid='date-range-filter']", "data-selected") == "30"
        ...
        if team_filter:
            ...
            selected_member = await page.text_content("[data-testid='team-filter'] .selected")
    ...
    async def test_navigation_flow(self, page: Page, base_url: str):
        ...
        for tab_id in nav_sections:
            ...
            if tab:
                ...
                section_content = await page.query_selector(f"[data-testid='{tab_id.replace('-tab', '-content')}']")
                if section_content:
                    assert await section_content.is_visible(), f"Section content for {tab_id} not visible"

    @pytest.mark.e2e
    async def test_data_loading_states(self, page: Page, base_url: str):
        """Test loading states and error handling."""
        # Intercept API calls to simulate loading states
        await page.route("**/api/metrics", lambda route: route.fulfill(
            status=200,
    ...
    async def test_performance_metrics(self, page: Page, base_url: str):
        ...
        if start_time and load_time:
            ...
            assert page_load_time < 10000, f"Page load time too slow: {page_load_time}ms"
        ...
        assert content_ready_time < 15000, f"Content ready time too slow: {content_ready_time}ms"
