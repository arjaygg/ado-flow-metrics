...
class TestFlowMetricsCalculator:
    ...
    def test_calculate_lead_time_multiple_items(self, calculator, sample_work_items_with_history):
    ...
    def test_calculate_cycle_time_with_work_states(self, calculator):
        ...
        cycle_time = calculator.calculate_cycle_time([work_item], work_states=work_states)
        
        assert cycle_time == 10.0  # 10 days from first work state to completion

    def test_calculate_throughput_weekly(self, calculator, sample_work_items_with_history):
        """Test throughput calculation on weekly basis."""
        throughput = calculator.calculate_throughput(
            sample_work_items_with_history, 
            period_days=7
        )
    ...
    def test_calculate_throughput_by_story_points(self, calculator, sample_work_items_with_history):
        """Test throughput calculation by story points."""
        throughput = calculator.calculate_throughput(
            sample_work_items_with_history,
            period_days=7,
            by_story_points=True
        )
        
        # Completed items: 8 + 5 = 13 story points in ~2 weeks = 6.5 points per week
        assert throughput == 6.5

    def test_calculate_work_in_progress(self, calculator, sample_work_items_with_history):
    ...
    def test_calculate_flow_efficiency(self, calculator):
        ...
        work_item = {
            ...
            "state_transitions": [
                {"state": "New", "date": "2024-01-01T00:00:00Z"},
                {"state": "Active", "date": "2024-01-05T00:00:00Z"},     # Work time: 5 days
                {"state": "Blocked", "date": "2024-01-10T00:00:00Z"},   # Wait time: 3 days  
                {"state": "Active", "date": "2024-01-13T00:00:00Z"},    # Work time: 2 days
                {"state": "Done", "date": "2024-01-15T00:00:00Z"}
            ]
        }
            ...
            for item_id in range(3):  # 3 items per week
                work_items.append({
                    ...
                    "state_transitions": [
                        ...
                        {
                            "state": "Done", 
                            "date": (base_date + timedelta(weeks=week, days=5)).isoformat()
                        }
                    ]
                })
        ...
        work_items = [
            {
                ...
                "state_transitions": [
                    ...
                    {"state": "Review", "date": "2024-01-10T00:00:00Z"},       # 5 days (bottleneck)
            ...
            },
            {
                ...
                "state_transitions": [
                    ...
                    {"state": "Review", "date": "2024-01-12T00:00:00Z"},       # 6 days (bottleneck)
        ...
        for i in range(1000):  # 1000 work items
            large_dataset.append({
                ...
                "state_transitions": [
                    ...
                    {"state": "Active", "date": (base_date + timedelta(days=1)).isoformat()},
                    {"state": "Done", "date": (base_date + timedelta(days=5)).isoformat()}
                ] if i % 2 == 0 else [
                    {"state": "New", "date": base_date.isoformat()},
                    {"state": "Active", "date": (base_date + timedelta(days=1)).isoformat()}
                ]
    ...
    @pytest.mark.parametrize("state_config", [
